...
table = np.array(['年級', '編號', '班別', '科目名稱', '任課教授', '上課時數\n正課/實驗實習/書報討論', '學分', '選必',
       '上課時間', '上課地點', '限修人數', '開課學制', '課程大綱'])
DepartMent = ""
S = True
while S == True:
    driver.get("https://kiki.ccu.edu.tw/~ccmisp06/Course/index.html")
    ...
    try:
        ...
        print("沒有符合的科系。")
        continue
    sub_table = find_column(1)
    for j in range(2,14):
        sub_table = np.hstack((sub_table,find_column(j)))
    table = np.vstack((table,sub_table[1:,:]))
